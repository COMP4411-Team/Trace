...
/*
class metaSphere: public Sphere
{
public:
	metaSphere(Scene* scene, Material* material, const vec3f& pos, const vec3f& target, double radius, double time0, double time1) :
		Sphere(scene, material), pos(pos), target(target), radius(radius), time0(time0), time1(time1) { }

	bool intersect(const Ray& r, Isect& i, Isect& i2);
	bool hasBoundingBoxCapability() const override { return false; }
	void ComputeBoundingBox() override { }
